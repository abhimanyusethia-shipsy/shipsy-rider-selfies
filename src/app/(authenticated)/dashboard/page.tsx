    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-4 gap-4">
        <div className="bg-white rounded-xl shadow-sm px-5 py-4 flex items-center gap-4">
          ...
          <div>
            <p className="text-xs text-gray-400 font-medium uppercase tracking-wide">Selfies Processed</p>
            <p className="text-2xl font-bold text-gray-800 leading-tight">{stats?.total ?? "—"}</p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm px-5 py-4 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-2xl font-bold text-gray-800 leading-tight">{stats?.uniqueUsers ?? "—"}</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm p-4">
        <form onSubmit={handleSearch} className="flex items-center gap-3 flex-wrap row-gap-2">
          <input
            type="text"
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            placeholder="Search worker name..."
            className="px-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none w-52"
          />
          <select
            value={statusFilter}
            onChange={(e) => {
          ...
          <button
            type="submit"
            className="px-6 py-2 rounded-lg text-white text-sm font-medium cursor-pointer"
          ...
          {(dateFrom || dateTo || search || statusFilter !== "all") && (
            <button
              ...
              className="px-4 py-2 rounded-lg text-sm font-medium text-gray-500 border border-gray-300 hover:bg-gray-50 cursor-pointer"
            >
              Clear
            </button>
          )}
            ...
            {data.totalPages > 1 && (
              <div className="flex items-center justify-between px-4 py-3 border-t border-gray-100">
                <button
                  ...
                  disabled={page === 1}
                  className="px-4 py-1.5 text-sm border border-gray-300 rounded-lg disabled:opacity-50 cursor-pointer hover:bg-gray-50"
                >
                  Previous
                </button>
